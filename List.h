...
void* front(List L);          // Returns the front element of L. PRECONDITION: length(L)>0.
void* back(List L);           // Returns the back element of L. PRECONDITION: length(L)>0.
void* get(List L);            // Returns the cursor element of L. PRECONDITION: length(L)>0, index(L)>=0.

bool isEmpty(List L);       // Returns true if L is empty, false otherwise.

// Manipulation procedures ----------------------------------------------------
void clear(List L);         // Resets L to its original empty state.
void set(List L, void* x);     // Overwrites the cursor element with x. PRECONDITION: length(L)>0, index(L)>=0.
...
void prepend(List L, void* x); // Insert a new element into L. If L is non-empty, insertion takes place before the front element.
void append(List L, void* x);  // Insert a new element into L. If L is non-empty, insertion takes place after the back element.
void insertBefore(List L, void* x);  // Insert a new element before the cursor. PRECONDITION: length(L)>0, index(L)>=0.
void insertAfter(List L, void* x);   // Insert a new element after the cursor. PRECONDITION: length(L)>0, index(L)>=0.
